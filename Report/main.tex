...
To ensure real-time performance, many optimizations 
...
in favor of a more responsive and light behaviour.  
\begin{itemize}
    ...
    \item \textbf{Sparse sampling:} When a sample is generated too close to an existing node, it is discarded and a new sample is generated.
            ...
            However, this threshold must be small enough to allow the tree to explore narrow passages. This feature may reduce the bias introduced by K-nearest rewiring, 
            since a less dense tree will favor wider (and better) rewirings.
    \item \textbf{Path sampling:} When a path already exists, a configurable percentage of samples are generated around the existing path instead of the global disk.
            This focuses exploration on improving the existing path, which is likely to be close to optimal already. The rest of the samples are still generated in the global disk
            to allow discovering new paths.
    ...
    \item \textbf{Adaptive sampling:} Samples are generated uniformly within a disk around the agent and the current goal, focusing exploration on relevant areas. The size of the disk is configurable,
    ...
    \item \textbf{Quick build:} Each time the tree is rebuilt, a large number of samples is generated to quickly find an initial path. 
            Each new sample tests the goal, and the process stops as soon as a path is found. Then, subsequent refinements will use fewer samples to optimize the existing tree in a goal-agnostic way.
    ...
    \item \textbf{Tree size limit:} To reduce the tree-search space, there are multiple maximum point amounts depending on whether
           a path has been found or not. When there are no paths, the tree can grow larger to explore more space. When the map changes
           this amount is also increased temporarily to allow finding new paths.
\end{itemize}
...
selects its second endpoint as the target to reach.

To prevent the agent from getting stuck at the endpoints, if the next segment endpoint has no 
obstructions the agent skips to it directly.
...
[Figure with ND rays + goal, an obstacle, the selected valley, the repulsion vectors and the final direction]
...
By just adding 10 new points per frame, RTT* + ND is able 
...
The navigation pipeline takes around 0.6ms per frame, mostly inside
...
generated per frame by RTT* is reduced to 3 to maintain performance,
at the cost of increasing the time needed to find a path.

The scene runs mostly at 144fps, although the frame time 
(figure \ref{fig:frameTime}) remains unstable due
to the high number of agents querying the navigation pipeline simultaneously.
Most of the time is spent again inside the get-k-nearest function,
so a C++ implementation would significantly improve stability.
Even with that, the frame time consistently remains around or under 16ms.
...
\begin{figure}[!ht]
    ...
    The red line marks 16ms, the budget for 60fps.
    }
    \label{fig:frameTime}
\end{figure}

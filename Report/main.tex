...
This work presents a real-time navigation stack for 2D videogame 
agents in Godot that plans directly in continuous space without 
hand-authored navigation graphs. It combines a customized, 
goal-agnostic RRT* global planner with a discretized Nearness Diagram (ND) local 
controller for short-horizon obstacle avoidance, and is 
demonstrated in a maze platformer game with dynamic environment changes.
Across hand-crafted and procedural mazes, the system 
produces consistent pursuit behaviour with collision avoidance and 
remains lightweight enough for real-time use with many
simultaneous agents. Results indicate that pairing 
sampling-based global planning with inexpensive reactive 
control yields smooth, responsive navigation under dynamic 
changes while meeting tight per-tick runtime budgets, 
while long-range cases remain limited by the sampling 
heuristics and tree-size constraints.

\end{abstract}

\noindent\textbf{Keywords---} Sampling-based planning; RRT*; local obstacle avoidance; Nearness Diagram; game AI; Godot.
...
legible and consistent within the game's fiction. From enemy flanking in 
...
rendering, physics, AI, and other systems, all that using frameworks that typically have very limited multithreading capabilities
...
algorithm acts as the global planner, computing nominal (line-of-sight checked) paths in continuous space, while an 
...
Game navigation is commonly built on precomputed navigation meshes 
and graph search, typically A* \cite{hart_a_star_1968}. 
Navigation meshes were popularized for games as a designer-friendly 
representation that supports robust pathfinding while keeping 
runtime queries cheap. These approaches 
work well when the world is mostly static and when authoring or
baking navigation data is acceptable.

In robotics, probabilistic and sampling-based planners 
such as RRT \cite{lavalle_rrt_1998} and 
RRT* \cite{karaman_rrtstar} enable planning in continuous spaces 
without an explicit grid, at the cost of stochastic convergence 
and potentially higher runtime variance. For short-horizon 
collision avoidance, many controllers act 
reactively using local sensing. The dynamic window approach 
(DWA) \cite{fox_dwa_1997} selects admissible velocities under 
dynamic constraints.

Our work follows a common hybrid pattern: a global sampling-based 
planner generates a coarse route, while an inexpensive reactive 
local layer refines motion near obstacles. The implementation 
emphasis is on game-engine integration and tight per-tick 
budgets rather than theoretical optimality guarantees.

\subsection{Bob in the candyland}

A videogame has been created to showcase the navigation pipeline (figure \ref{fig:candyland_menu}).
It is a 2D platformer where Bob the triceratops (the player) has to navigate through a 
procedurally generated candy maze to reach the exit. The maze has bat enemies (the agents) that pursue the player using the navigation pipeline described in this report.

To make the challenge more interesting the player has a special ability that 
allows to destroy walls to create new paths, making the maze a dynamic environment.

In addition, it also includes a test level used in this report to benchmark the navigation pipeline.
It is fully implemented in GDscript (a python-like language), allowing easy deployment and modification, 
and the release can be downloaded from the project repository \cite{arob_repo}.
...
Maintaining the latency target consistently is crucial 
...
which offers the physics and rendering capabilities needed for the simulation.
...
\begin{itemize}            
    \item \textbf{Global planner:} Computes a nominal path from the agent's current position to a goal position using a custom RRT* variant. Edges are validated with ray-casts (line-of-sight), while full-footprint clearance is refined by the local planner.
    ...
    \item \textbf{Local planner:} A discretized holonomic ND \cite{minguez_nd_2004} controller serves as the local planner. 
    ...
    \item \textbf{Low-level controller:} It receives the commands from the 
...
If the nominal route becomes invalid due to a moving obstacle, the local planner will need to adjust locally and/or trigger a full global replan.
...
However that is impossible to implement in the physics engine, since it would require to manage two independent collision shapes per object that may change dynamically.
...
Instead, candidate edges are checked using simple ray-casts (line-of-sight for a point), and footprint-level clearance is left to the local planner. 
As a result, the global planner should be understood as generating a nominal route rather than providing a strict collision-free guarantee for the agent's full shape.
...
\begin{itemize}
    \item \textbf{Adaptive sampling:} Samples are generated uniformly within a disk around the agent and the current goal, 
            ...
            this makes impossible to find paths that go outside the disk, 
            so more complex heuristics could be designed 
            to adaptively increase the disk size when no paths are found.
    ...
    \item \textbf{Tree size limit:} To reduce the tree-search space, there is a dynamic maximum point amount 
            ...
            of path complexity that can be found, but it provides hard bounds
...
\begin{itemize}
    \item \textbf{K-nearest rewiring:} When a new node is added, only the k-nearest neighbors \cite{cover_hart_1967} are considered for rewiring, 
    ...
    \item \textbf{Efficient nearest neighbor search:} A k-d tree structure\cite{bentley_kdtree_1975} is used to allow fast nearest neighbor queries during tree expansion. 
...
change forward. If an aggressive path change makes the closest segment behind the agent,
then a replanning will be triggered to prevent oscillations.
...
Before performing the valley search, the rays are sorted by their angular distance to the goal direction.
...
\section{Low-level controller}\label{sec:lowlevelcontrol}

The low-level controller is responsible for executing the movement commands provided by the local planner.
...
\subsection{Adapting the stack to ROS}

Adapting this work to ROS~2\cite{macenski_ros2_2022} mostly means formalizing the interfaces 
between layers and running each component as a node (or a plugin)
with clear contracts, rather than as in-engine scripts. 
The core algorithms remain the same, but perception, state, 
commands, and time become explicit data streams with well-defined rates and coordinate frames.

In a ROS~2 navigation setup, the cleanest mapping is to 
integrate with the \emph{Navigation2} (Nav2) architecture:
\begin{itemize}
    \item \textbf{Global planner $\rightarrow$ Nav2 global-planner plugin:} Wrap the RRT* 
    planner as a \texttt{nav2\_core::GlobalPlanner} plugin. Input 
    is the current pose and goal pose in a global frame (e.g., \texttt{map}); 
    output is a \texttt{nav\_msgs/Path}. Collision checking moves from 
    Godot ray-casts to queries over a costmap (inflation for footprint clearance) 
    plus optional segment validity checks.

    \item \textbf{Trajectory follower $\rightarrow$ controller input shaping:} The waypoint 
    selection logic becomes the ``carrot'' or lookahead target that feeds 
    the controller at a fixed control rate. This can live inside the 
    controller plugin (preferred for tight timing) or as a small node 
    that converts \texttt{nav\_msgs/Path} into intermediate targets.

    \item \textbf{Local planner $\rightarrow$ Nav2 controller plugin:} Implement 
    the discretized holonomic ND as a \texttt{nav2\_core::Controller} 
    plugin producing \texttt{geometry\_msgs/Twist}. Replace the in-engine 
    sensing with costmap-based obstacle distances (or a laser/pointcloud pipeline), 
    keeping the same sector discretization and selection rule. This section becomes
    less relevant thanks to the possibility of using obstacle inflation in the global planner.

    \item \textbf{Low-level control $\rightarrow$ robot base interface:} The game-specific 
    movement (acceleration/jump) becomes the robot platform controller (diff-drive/holonomic base). 
    For non-holonomic platforms, the ND selection must be constrained to 
    feasible velocities, or coupled with a kinematic layer (e.g., DWA-style admissibility filtering).
\end{itemize}

Operationally, the adaptation requires (i) a consistent 
TF tree (\texttt{map}$\rightarrow$\texttt{odom}$\rightarrow$\texttt{base\_link}),
(ii) a source of localization/odometry, and (iii) 
an obstacle representation (local/global costmaps).
For multi-agent pursuit, each agent runs the same stack under a
namespace (\texttt{/bat\_i/...}) with isolated TF, costmaps, and 
parameters, while sharing global static data (map) when applicable. 
Finally, debugging and evaluation move naturally to ROS tooling: 
RViz for path/TF visualization and rosbag2 for recording trajectories, 
replans, and timing to reproduce corner cases.
...
to find a path through the maze in an average of 3.7 seconds (in our demo scene).
...
In most short and medium cases, the path is found fast and nearly optimally.
...
of them aggregated, over Godot 4.5.1 running on a Ryzen 9 HX 370, 
32 gb of DDR5 RAM and an RTX 5080 laptop GPU.
...
The main limitations arise from bounding the sampling region and limiting the tree size, which can prevent discovering long paths around large obstacles.
Future work could include adaptive expansion of the sampling domain, better informed sampling heuristics for large-scale mazes, better tree reusage during environment changes and moving performance-critical components (e.g. kd-tree) to a C++ implementation to further reduce runtime overhead under heavy multi-agent load.
...
\subsection{Content and tools}

Most of the graphic art used in the project comes from free online resources in Itch.io,
and the rest has been created by the team using Canva and Gimp. The music and sound effects have been created using the AI tool Suno \cite{suno_ai}.

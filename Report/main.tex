...
\begin{itemize}
    ...
    \item \textbf{Tree size limit:} To prevent the tree from growing indefinitely, a maximum size is enforced. 
            When the limit is reached, the tree is rebuilt from the agent's current position. This ensures that the planner remains responsive and does not degrade over time.
\end{itemize}
...
Figure \ref{fig:profiler} shows the Godot profiler when running the demo maze.
The navigation pipeline takes around 1ms per frame, mostly inside
the get-k-nearest function. This is a heavily algorithmic function 
with a high penalty coming from GDscript's interpreted nature. 

\begin{figure}[!ht]
    \centering
    \includegraphics[width=0.6\columnwidth]{Images/profiler.png}
    \caption{Godot profiler when running the demo maze.
    }
    \label{fig:profiler}
\end{figure}

Figure \ref{fig:profiler} shows a spike around the half of the timeline, which corresponds to
the opening of a new path while the agent is too far from the origin, triggering a new
tree rebuild.

The same code written in C++ runs around 10x faster, resulting in an 
average of just 0.1ms per frame. This code has not been 
included in the final release due to the complexity of 
cross-compiling C++ modules to different platforms.

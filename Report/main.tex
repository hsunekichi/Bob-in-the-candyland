...
goal-agnostic RRT* global planner with a simple trajectory 
follower and a discretized Nearness Diagram (ND) local 
controller for short-horizon obstacle avoidance, and is 
demonstrated in a maze-like platformer game with dynamic layout changes.
...
and search algorithms like A* \cite{hart_a_star_1968}, which require a carefully constructed graph discretizing the free space.
...
procedurally generated candy maze to reach the end. The maze has bat enemies (the agents) that pursue the player 
...
allows to destroy walls to create new paths in the maze, making it a dynamic environment.

In addition, it also includes a debug level used in this report to test and visualize the navigation pipeline.
It is fully implemented in GDscript (a python-like language), allowing easy deployment and modification, 
and the releases can be downloaded from the project repository \cite{arob_repo}.
...
Maintaining this latency target consistently is crucial 
...
The main limitations arise from bounded sampling regions and capped tree sizes, which can prevent discovering long detours around large obstacles.
Future work includes adaptive expansion of the sampling domain, more informed sampling heuristics for large-scale mazes, and moving performance-critical components (e.g., nearest-neighbor queries) to a compiled implementation to further reduce runtime variance under heavy multi-agent load.

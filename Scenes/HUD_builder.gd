...
@export var _templates: Array[Node] = []
...
func _apply_random_transform(clone: Node) -> void:
	...
	var offset := _random_offset()

	if clone is Control:
		var control := clone as Control
		control.rect_rotation = rotation_deg
		control.rect_scale *= scale_factor
		control.rect_position += offset
		return
...
func _random_offset() -> Vector2:
	return Vector2(
		_rng.randf_range(-abs(offset_range.x), abs(offset_range.x)),
		_rng.randf_range(-abs(offset_range.y), abs(offset_range.y))
	)
